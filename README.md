...
- **高度な最適化**:
    ...
    - 科目レベル（番台）によるフィルタリング
- **対話型編集**: 生成された候補を選択し、その場で科目の追加・削除を行って微調整できます。
- **カレンダー出力**: 確定した時間割をiCalendar形式（.ics）で出力し、GoogleカレンダーやOutlookにインポート可能です。変則的な授業時間（キリスト教週間やロングコマなど）にも対応しています。
...
- `save`: 現在の時間割を `.ics` ファイルに出力して保存します。
- `back`: 候補選択画面に戻ります。
